...
{
    public class FactionPermSettingsWindow : CyberFormCustom
    {
        public FactionPermSettingsWindow(Faction f) : base(MainForm.Faction_Settings_Window)
        {
        ...
        {
            Text = "Allow Allies To Teleport To Your Faction Homes",
            Value = fs.AllowAlliesToTPToHomes
        });
        ...
        ExecuteAction = delegate(Player player, CustomForm form)
        {
            ...
            bool aatt = ((Toggle)c[10]).Value;
            float wft = ((Slider) c[11]).Value;
            FactionPermSettings fs = f.getPermSettings();
            ...
            fs.AllowAlliesToTPToHomes = aatt;
            fs.setWeeklyFactionTax((int)wft);
            f.getSettings().upload();

            player.SendMessage("Faction's Permission Settings Have Been Updated!");
        };
        }
    }
