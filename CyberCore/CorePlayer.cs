...
namespace CyberCore
{
    ...
    private static bool CooldownLock = false;
    ...
    // protected Dictionary<Buff.BuffType, Float> lastdata = null;
    long uct = 0;
    bool uw = false;
    // private FormWindow nw;
    private Item ItemBeingEnchanted = null;
    private bool ItemBeingEnchantedLock = false;
    private PlayerSettingsData SettingsData = null;
    private RankList rank = RankList.PERM_GUEST;
    private Vector3 lastBreakPosition1 = new Vector3();
    private Vector3 CTLastPos = new Vector3();
    ...
    private Dictionary<String, CoolDown> CDL = new Dictionary<String, CoolDown>();
    // private CustomCraftingTransaction cct;
    private Dictionary<BuffOrigin, Dictionary<Buff.BuffType, Buff>> Bufflist = new Dictionary<BuffOrigin, Dictionary<Buff.BuffType, Buff>>() {{
        put(BuffOrigin.Class, new Dictionary<>());
    }};
    private Dictionary<BuffOrigin, Dictionary<Buff.BuffType, DeBuff>> DeBufflist = new Dictionary<BuffOrigin, Dictionary<Buff.BuffType, DeBuff>>() {{
        put(BuffOrigin.Class, new Dictionary<>());
    }};
    ...
    public Dictionary<BuffOrigin, Dictionary<Buff.BuffType, Buff>> getBufflist() {
        return (Dictionary<BuffOrigin, Dictionary<Buff.BuffType, Buff>>) Bufflist.clone();
    }

    public Dictionary<BuffOrigin, Dictionary<Buff.BuffType, DeBuff>> getDeBufflist() {
        return (Dictionary<BuffOrigin, Dictionary<Buff.BuffType, DeBuff>>) DeBufflist.clone();
    }

    public Dictionary<Buff.BuffType, DeBuff> getClassDeBuffList() {
        if (!getBufflist().containsKey(BuffOrigin.Class)) return new Dictionary<>();
        return (Dictionary<Buff.BuffType, DeBuff>) getDeBufflist().get(BuffOrigin.Class).clone();
    }

    public bool takeExperience(int add) {
...
//        int t = 0;
    ...
    public Dictionary<Buff.BuffType, Buff> getClassBuffList() {
        if (!getBufflist().containsKey(BuffOrigin.Class)) return new Dictionary<>();
        return (Dictionary<Buff.BuffType, Buff>) getBufflist().get(BuffOrigin.Class).clone();
    }

    public Dictionary<Buff.BuffType, Buff> getTempBuff() {
        if (!getBufflist().containsKey(BuffOrigin.Temp)) return new Dictionary<>();
        return (Dictionary<Buff.BuffType, Buff>) getBufflist().get(BuffOrigin.Temp).clone();
    }
    ...
    public void addTemporaryBuff(Buff b, Object... oo) {
        if (Bufflist.containsKey(BuffOrigin.Temp)) {
            Dictionary<Buff.BuffType, Buff> o = Bufflist.get(BuffOrigin.Temp);
            o.put(b.getBt(), b);
            ...
            Bufflist.put(BuffOrigin.Temp, new Dictionary<Buff.BuffType, Buff>() {{
                put(b.getBt(), b);
            }});
        }
    }
...
//                    break;
    ...
    public void initBuffs() {
        ...
        Dictionary<Buff.BuffType, Float> data = new Dictionary<>();
        //BUFFS
        List<Buff> ab = new List<>();
...
//        ab.addAll(getTempBuff().values());
    ...
    private bool areequal(Dictionary<Buff.BuffType, Float> a, Dictionary<Buff.BuffType, Float> b) {
    ...
    public void addBuffFromClass(Buff b, bool force) {
        ...
        if (!Bufflist.containsKey(BuffOrigin.Class) || Bufflist.get(BuffOrigin.Class) == null) {
            Dictionary<Buff.BuffType, Buff> hash = new Dictionary<Buff.BuffType, Buff>();
            hash.put(b.getBt(), b);
            Bufflist.put(BuffOrigin.Class, hash);
        } else {
            Dictionary<Buff.BuffType, Buff> bm = Bufflist.get(BuffOrigin.Class);
    ...
    public void addDeBuffFromClass(DeBuff b, bool force) {
        ...
        if (!DeBufflist.containsKey(BuffOrigin.Class) || DeBufflist.get(BuffOrigin.Class) == null) {
            Dictionary<Buff.BuffType, DeBuff> hash = new Dictionary<Buff.BuffType, DeBuff>();
            hash.put(b.getBt(), b);
            DeBufflist.put(BuffOrigin.Class, hash);
        } else {
            Dictionary<Buff.BuffType, DeBuff> bm = DeBufflist.get(BuffOrigin.Class);
            if (bm.containsKey(b.getBt()) && !force) return;//Cant write it something is here
            bm.put(b.getBt(), b);
...
//            TODO
            ...
            if (bc != null) {
                ...
                if (t != null && t.size() != 0) {
                    sd.addLine("Class:", k++);
                    for (System.String ss : t) sd.addLine("    " + ss, k++);
                } else {
...
//        }
    ...
    private CoolDownTick GetCooldown(String key, bool checkvalid) {
...
//            }
            ...
            if (!this.isSpectator() && this.speed != null) {
                if (this.onGround) {
                    ...
                    if (this.checkMovement && !this.isGliding() && !server.getAllowFlight() && !this.getAdventureSettings().get(AdventureSettings.Type.ALLOW_FLIGHT) && this.inAirTicks > 20 && !this.isSleeping() && !this.isImmobile() && !this.isSwimming() && this.riding == null) {
                        ...
                        bool ignore = block == Block.LADDER || block == Block.VINES || block == Block.COBWEB;
    ...
    public bool CheckHomeKey(String key) {
        for (Manager.Factions.Faction.HomeData h : HD) {
...
//    }
    ...
    public void completeLoginSequence() {
        ...
        this.server.getLogger().info(this.getServer().getLanguage().translateString("nukkit.player.logIn",
                ...
                this.ip,
                System.String.valueOf(this.port),
                System.String.valueOf(this.id),
                this.level.getName(),
                System.String.valueOf(NukkitMath.round(this.x, 4)),
                System.String.valueOf(NukkitMath.round(this.y, 4)),
                System.String.valueOf(NukkitMath.round(this.z, 4))));
...
//        System.out.println("PRINGING A LINE SEEEEEEEEEEEEEEEEEEEEE");
    ...
    public bool delayTeleport(int i, Position pos, bool giveeffects) {
        return delayTeleport(i,pos,giveeffects,2);
    }
    public bool delayTeleport(int i, Position pos) {
        return delayTeleport(i,pos,true,2);
    }

    public bool delayTeleport(int i, Position pos, bool giveeffects, int canceltpdistance) {
        if (isWaitingForTeleport()) {
    ...
    public bool WaitingForTPEffects = false;
